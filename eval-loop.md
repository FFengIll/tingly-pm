...
Every round requires an **exploration seed** — a short text describing the direction or focus for this round. The seed can be provided by the user via `eval-loop.sh -d`, or **generated by you at the start of the round** if none is provided in the prompt.

A seed is a compass, not a constraint. It guides fixture selection and hypothesis formation, but can be ignored if baseline results reveal more pressing issues.

### Seed Generation

When no seed is provided in the prompt (no `EXPLORATION SEED` line), generate one before running the baseline:

1. **Review the previous round's final report** (if any) — check "Next Round Recommendations"
2. **Review open issues** — check improvement log, discovered gaps
3. **Skim the agent's current prompt and tools** — look for areas that haven't been tested recently
4. **Write a 1-2 sentence seed** that suggests a direction, e.g.:
   - "focus on cross-language duplicate detection"
   - "explore edge cases in multi-turn context resolution"
   - "investigate behavior when task list is empty"

Record the seed (generated or provided) in the baseline report.
...
- **Seed-guided** — every round has a seed (provided or generated). It shapes which fixtures you sample and what hypotheses you form, but can be ignored if baseline reveals more pressing issues.
...
The exploration seed (user-provided via `-d` or generated by you) shapes baseline fixture selection:
...
**Seed:** [seed text] (provided / generated)
**Rationale:** [why this seed, if generated]
